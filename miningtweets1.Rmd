...
pivot_wider(names_from = month, values_from = proportion)  %>%  #gather(month, proportion, 3:4)
pivot_longer(3:4, names_to = "month", values_to = "proportion")

frequency
#save(frequency, file="./data/frequency.rda")
```

We use str_extract() here because the UTF-8 encoded tweet texts have some examples of words with underscores around and other signs. The tokenizer treated these as words, but we don’t want to count “_any_” separately from “any” as we saw in our initial data exploration before choosing to use str_extract().
